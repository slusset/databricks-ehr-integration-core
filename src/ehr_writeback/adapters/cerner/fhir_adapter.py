"""Cerner/Oracle Health FHIR R4 Observation.Create adapter.

Cerner's FHIR implementation differs from Epic in several ways:
- Different OAuth flow (client_credentials with client_secret, not JWT)
- category coding uses slightly different value sets in practice
- Reference format for Patient may use Cerner's person ID
"""
...
from ehr_writeback.adapters.cerner.auth import CernerOAuth2Auth
...
@dataclass
class CernerFHIRAdapter(EHRWritebackPort):
    """Writes observations to Cerner via FHIR R4 Observation.Create."""

    base_url: str  # e.g. https://fhir-open.cerner.com/r4/{tenant_id}
    auth: CernerOAuth2Auth
    ...
    def _idempotency_key(obs: Observation) -> str:
        raw = f"cerner:{obs.patient_id}:{obs.code}:{obs.effective_datetime.isoformat()}"
